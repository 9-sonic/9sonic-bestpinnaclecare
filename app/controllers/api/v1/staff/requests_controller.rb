    ...
    module Staff
      ...
      class RequestsController < BaseController
        ...
        def create
          ...
          # A carer can only raise a drop — that's the only kind the app supports.
          req = current_employee.carer_requests.create!(request_params.merge(kind: "drop", state: "pending"))
        ...
        # A carer can't drop a visit that's no longer theirs to hand back — one
        # that's already completed, missed or cancelled. The shift already
        # happened (or was withdrawn); there is nothing left to cover.
        def drop_error
