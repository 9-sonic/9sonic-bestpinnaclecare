...
RSpec.describe "Admin cover", type: :request do
  ...
  it "accepts cover for a carer already on an overlapping visit — carers may double up" do
    v = open_visit
    ...
    post "/api/v1/admin/cover_offers/#{offer_id}/accept", headers: auth
    expect(response).to have_http_status(:ok)
    expect(VisitAssignment.where(visit: v, assignment_status: "assigned")).to exist
    expect(employee.visit_assignments.assigned.count).to eq(2) # both, overlapping
  end
