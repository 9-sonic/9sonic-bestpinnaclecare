...
```bash
npm test          # everything, phone and desktop
npm run test:ui   # interactive runner, good for debugging a failure
npm run check     # build then test, run this before pushing
```

Playwright drives a real browser against a production build it makes itself and
serves on port 5310, so it never clashes with a dev server you have open.

Testing the build rather than the dev server is deliberate. The dev server
compiles each route the first time it is asked for, and with several workers
signing in at once that queue was slow enough to time out tests that were
otherwise fine. Building first also means the suite exercises what actually
ships.

### What the tests are for

They are not there for coverage. Each one locks in something that has already
broken once, because none of these were catchable by the build, the linter or a
type checker. They only show up when the app is rendered and looked at:

| Test | The bug it prevents coming back |
|---|---|
| Bottom nav has five evenly spaced cells | The clock button was positioned out of the grid, so the other tabs slid left and put the wrong label under the wrong icon |
| Clock button floats and stays round | `contain: paint` on the bar clipped the top off the button |
| Home keeps its grid and shortcuts | The card grid was replaced with a flat strip and the section below it left empty |
| Shift cards are a time column beside details | The clock screen kept its own copy of the card markup, which collapsed into an unreadable vertical strip when the shared card became a grid |
| Long database values never push the page sideways | Real names and addresses are longer than the design assumed |
| Back closes sheets and the drawer | React StrictMode double-invoked the history effect, so back exited the screen instead of closing the sheet |
| Sticky headers stay put | An entry animation on an ancestor silently breaks `position: sticky` |
| Touch targets and labels | Controls drifting under 32px or losing their accessible name |
| Every route renders and logs nothing | Any screen throwing to the error boundary |

Adding a screen means adding it to `ROUTES` in `tests/helpers.js`. That is the
only maintenance the suite needs day to day.

### When something breaks

`npx playwright show-trace test-results/<the-failing-test>/trace.zip` replays
the run with a DOM snapshot at every step, which is usually faster than
reproducing the failure by hand.

## Testing on your phone

The dev server listens on your network, so a phone on the same Wi-Fi can open
it directly.

1. Start it: `npm run dev`
2. Vite prints a **Network** address, for example `http://192.168.100.128:5173/`
3. Type that into your phone's browser

If it does not connect, the usual causes are:

- **Windows Firewall.** The first run should prompt to allow Node; if you
  dismissed it, allow it manually for private networks.
- **The wrong address.** A machine with VirtualBox or VMware installed shows
  several. The one on the same subnet as your phone is the one you want.
- **Guest Wi-Fi or client isolation.** Many routers stop devices talking to each
  other on a guest network. Use the main network.

Geolocation and passkeys need a secure context, so on a plain `http://` address
the browser will refuse them. Clocking in still works, it just records no
location. To test those properly, tunnel it instead:

```bash
npx ngrok http 5173
```

Then add the ngrok host to `server.allowedHosts` in `vite.config.js`.
